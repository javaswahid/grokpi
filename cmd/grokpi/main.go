...
import (
	...
	"github.com/javaswahid/grokpi/internal/cfrefresh"
	...
	"github.com/javaswahid/grokpi/internal/httpapi/openai"
...
func main() {
	...
	// DB ping func for health/readiness probes (with timeout in handlers)
	...
	tokenSvc.Manager().SetProxyProvider(func() *config.ProxyConfig {
		return &runtimeCfg.Get().Proxy
	})
	...
	healthChecker := token.NewHealthChecker(tokenSvc.Manager(), &cfg.Token, "https://grok.com")
	healthChecker.SetConfigProvider(func() *config.TokenConfig {
		return &runtimeCfg.Get().Token
	})
	healthChecker.Start(rootCtx)
	logging.Info("token health checker started", "interval_minutes", runtimeCfg.Get().Token.TokenHealthCheckIntervalMinutes)
	...
	videoJobStore := store.NewVideoJobStore(db)
	...
	srv := httpapi.NewServer(&httpapi.ServerConfig{
		...
		VideoTester:     &videoCapabilityTester{tokenStore: tokenStore, tokenSyncer: tokenSvc, runtime: runtimeCfg},
...
type videoCapabilityTester struct {
	tokenStore  *store.TokenStore
	tokenSyncer *token.TokenService
	runtime     *config.Runtime
}

func (t *videoCapabilityTester) TestVideoCapability(ctx context.Context, id uint) (*store.Token, error) {
	tok, err := t.tokenStore.GetToken(ctx, id)
	if err != nil {
		return nil, err
	}
	client, err := newXAIClient(t.runtime, tok.Token, true)
	if err != nil {
		return nil, err
	}
	defer client.Close()
	...
	now := time.Now()
	_, err = client.CreateVideoPost(checkCtx, "GrokPi video capability probe, short cinematic rainy window motion.")
	status, capable := classifyVideoCapabilityResult(err)

	tok.UpstreamVideoCapable = &capable
	tok.LastVideoCheckAt = &now
	tok.LastVideoCheckStatus = status
	tok.LastVideoError = ""
	if err != nil {
		tok.LastVideoError = truncateForDB(err.Error(), 512)
	}
	switch status {
	case "video_capable":
		tok.LastValidatedAt = &now
		if tok.Status == store.TokenStatusExpired || tok.Status == store.TokenStatusInvalid || tok.Status == store.TokenStatusRefreshRequired || tok.Status == store.TokenStatusRefreshFailed {
			tok.Status = store.TokenStatusActive
			tok.StatusReason = ""
			tok.CoolUntil = nil
			tok.FailCount = 0
		}
	case "upstream_401":
		tok.Status = store.TokenStatusExpired
		tok.StatusReason = "video capability check returned upstream 401"
		tok.ExpiresAt = &now
	case "upstream_429", "video_cooldown":
		tok.Status = store.TokenStatusCooling
		tok.StatusReason = "video capability check returned cooldown/rate limit"
		coolUntil := now.Add(5 * time.Minute)
		tok.CoolUntil = &coolUntil
	}

	if err := t.tokenStore.UpdateToken(ctx, tok); err != nil {
		return nil, err
	}
	if t.tokenSyncer != nil {
		_ = t.tokenSyncer.SyncToken(ctx, id)
	}
	return tok, nil
}

func classifyVideoCapabilityResult(err error) (status string, capable bool) {
	if err == nil {
		return "video_capable", true
	}
	...
	case strings.Contains(msg, "status 401"):
		return "upstream_401", false
	case strings.Contains(msg, "status 403"):
		if strings.Contains(msg, "video") || strings.Contains(msg, "access") || strings.Contains(msg, "permission") {
			return "no_video_access", false
		}
		return "upstream_403", false
	case strings.Contains(msg, "status 429"):
		if strings.Contains(msg, "cooldown") {
			return "video_cooldown", false
		}
		return "upstream_429", false
	case strings.Contains(msg, "quota"):
		return "quota_exhausted", false
	case strings.Contains(msg, "missing post id"), strings.Contains(msg, "unmarshal"), strings.Contains(msg, "invalid"):
		return "invalid_video_response", false
	default:
		return "unknown_upstream_error", false
	}
}
