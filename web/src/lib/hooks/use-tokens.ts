...
export function useRevalidateToken() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (id: number) => api.post<Token>(`/tokens/${id}/revalidate`),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: tokenKeys.detail(id) })
      queryClient.invalidateQueries({ queryKey: tokenKeys.lists() })
    },
...
export function useReplaceToken() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: TokenReplaceRequest }) =>
      api.post<Token>(`/tokens/${id}/replace`, data),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: tokenKeys.detail(id) })
      queryClient.invalidateQueries({ queryKey: tokenKeys.lists() })
    },
...
export function useResetTokenUsage() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (id: number) => api.post<Token>(`/tokens/${id}/reset-usage`),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: tokenKeys.detail(id) })
      queryClient.invalidateQueries({ queryKey: tokenKeys.lists() })
    },
...
export function useTestVideoCapability() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (id: number) => api.post<Token>(`/tokens/${id}/test-video-capability`),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: tokenKeys.detail(id) })
      queryClient.invalidateQueries({ queryKey: tokenKeys.lists() })
    },
