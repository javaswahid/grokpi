...
export const tokenConfigSchema = z.object({
  ...
  max_chat_quota: z.number().int().min(0),
  max_image_quota: z.number().int().min(0),
  max_video_quota: z.number().int().min(0),
  allow_unlimited_quota: z.boolean(),
  token_health_check_interval_minutes: z.number().int().min(1),
  token_expiry_warning_hours: z.number().int().min(1),
  token_critical_expiry_hours: z.number().int().min(1),
  allow_token_auto_refresh: z.boolean(),
  require_official_refresh_flow: z.boolean(),
