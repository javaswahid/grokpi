...
import { RefreshCw, Pencil, Trash2, Ban, CircleCheck, MoreHorizontal, RotateCcw, KeyRound, Video } from 'lucide-react'
...
export function TokenRowActions({ token, onEdit, onDelete, onRevalidate, onRefresh, onReplace, onResetUsage, onTestVideoCapability, onToggleStatus }: TokenRowActionsProps) {
    ...
    <div className="flex items-center gap-1 justify-end min-w-max">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-40">
          ...
          <DropdownMenuItem onClick={() => onRevalidate(token)}>
            <RefreshCw className="mr-2 h-4 w-4" /> Revalidate Token
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onRefresh(token)}>
            <RefreshCw className="mr-2 h-4 w-4" /> Refresh Token
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onReplace(token)}>
            <KeyRound className="mr-2 h-4 w-4" /> Replace Token
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onResetUsage(token)}>
            <RotateCcw className="mr-2 h-4 w-4" /> Reset Local Usage
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onTestVideoCapability(token)}>
            <Video className="mr-2 h-4 w-4" /> Test Video Capability
          </DropdownMenuItem>
