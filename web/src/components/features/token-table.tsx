...
const statusColors: Record<TokenStatus, string> = {
  ...
  quota_exhausted: 'bg-orange-600',
  invalid: 'bg-purple-600',
  refresh_required: 'bg-pink-600',
  refresh_failed: 'bg-red-800',
}
...
export interface TokenTableProps {
  ...
  onRevalidate: (token: Token) => void
  onRefresh: (token: Token) => void
  onReplace: (token: Token) => void
  onResetUsage: (token: Token) => void
  onTestVideoCapability: (token: Token) => void
  onToggleStatus: (token: Token) => void
}

function TokenTableInner({
  tokens, selectedIds, onSelectionChange,
  onEdit, onDelete, onRevalidate, onRefresh, onReplace, onResetUsage, onTestVideoCapability, onToggleStatus,
}: TokenTableProps) {
                ...
                <TableRow className={cn(isSelected && 'bg-[rgba(0,0,0,0.04)]/50')}>
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-1">
                      <StatusBadge
                        status={token.status}
                        label={tokenStatusLabel(token.status, t)}
                        title={token.status_reason || token.warning_message || ''}
                      />
                      {token.warning_message && (
                        <span className="max-w-[180px] text-xs text-red-600">{token.warning_message}</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="w-[120px] whitespace-nowrap">
                    <TokenRowActions token={token} onEdit={onEdit} onDelete={onDelete} onRevalidate={onRevalidate} onRefresh={onRefresh} onReplace={onReplace} onResetUsage={onResetUsage} onTestVideoCapability={onTestVideoCapability} onToggleStatus={onToggleStatus} />
                  </TableCell>
                </TableRow>
                {isExpanded && (
...
function tokenStatusLabel(status: TokenStatus, t: ReturnType<typeof useTranslation>['t']): string {
  switch (status) {
    case 'active':
      return t.tokens.active
    case 'expired':
      return t.tokens.expired
    case 'cooling':
      return t.tokens.cooling
