...
func DefaultConfig() *Config {
	return &Config{
		...
		Token: TokenConfig{
			...
			DefaultChatQuota:                100,
			DefaultImageQuota:               100,
			DefaultVideoQuota:               200,
			MaxChatQuota:                    5000,
			MaxImageQuota:                   5000,
			MaxVideoQuota:                   1000,
			AllowUnlimitedQuota:             true,
			TokenHealthCheckIntervalMinutes: 15,
			TokenExpiryWarningHours:         24,
			TokenCriticalExpiryHours:        6,
			AllowTokenAutoRefresh:           true,
			RequireOfficialRefreshFlow:      true,
