...
func (f *VideoFlow) generateVideoViaChat(ctx context.Context, tok *store.Token, req *VideoRequest) (string, error) {
	...
	if trace := VideoTraceFromContext(ctx); trace != nil {
		trace.setParentPostID(parentPostID)
	}
	LogVideoStage(ctx, "upstream_job_created", "parent_post_id", parentPostID)
	...
	LogVideoStage(ctx, "upstream_submit_completed")
	LogVideoStage(ctx, "upstream_poll_started")
	...
	if trace := VideoTraceFromContext(ctx); trace != nil {
		trace.setVideoPostID(result.postID)
		trace.setFinalVideoURL(result.videoURL)
	}
	...
	LogVideoStage(ctx, "video_url_resolved", "video_url", videoURL)
