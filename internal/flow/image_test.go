...
func TestImageFlow_Generate_BlockedDoesNotRetrySamePrompt(t *testing.T) {
	tokenSvc := &mockTokenService{
		tokens: []*store.Token{
	...
	assert.Nil(t, resp)
	assert.Contains(t, err.Error(), "blocked")
	assert.Equal(t, []string{"tok-1"}, usedTokens)
}
...
func TestImageFlow_BlockedRecovery_DoesNotConsumeOrFanOut(t *testing.T) {
	tokenSvc := &mockTokenService{
		tokens: []*store.Token{
			{ID: 1, Token: "tok-1", Pool: "basic"},
			{ID: 2, Token: "tok-2", Pool: "basic"},
	...
	tokenSvc.mu.Lock()
	defer tokenSvc.mu.Unlock()
	assert.Len(t, tokenSvc.consumeCalls, 0, "policy-blocked prompt must not consume quota or retry on another token")
}
