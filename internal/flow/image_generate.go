...
func (f *ImageFlow) generateWithRecovery(
	...
	// Policy/safety blocks must not be retried with the same prompt on other
	// tokens. The caller will mark the token in policy quarantine and wait for
	// safe prompt repair before retrying.
	return nil, err
}
