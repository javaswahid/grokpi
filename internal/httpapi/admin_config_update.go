...
func handlePutConfig(cfg *config.Config, configStore *store.ConfigStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Token != nil {
			...
			if req.Token.MaxChatQuota != nil {
				cfg.Token.MaxChatQuota = *req.Token.MaxChatQuota
			}
			if req.Token.MaxImageQuota != nil {
				cfg.Token.MaxImageQuota = *req.Token.MaxImageQuota
			}
			if req.Token.MaxVideoQuota != nil {
				cfg.Token.MaxVideoQuota = *req.Token.MaxVideoQuota
			}
			if req.Token.AllowUnlimitedQuota != nil {
				cfg.Token.AllowUnlimitedQuota = *req.Token.AllowUnlimitedQuota
			}
			if req.Token.TokenHealthCheckIntervalMinutes != nil {
				cfg.Token.TokenHealthCheckIntervalMinutes = *req.Token.TokenHealthCheckIntervalMinutes
			}
			if req.Token.TokenExpiryWarningHours != nil {
				cfg.Token.TokenExpiryWarningHours = *req.Token.TokenExpiryWarningHours
			}
			if req.Token.TokenCriticalExpiryHours != nil {
				cfg.Token.TokenCriticalExpiryHours = *req.Token.TokenCriticalExpiryHours
			}
			if req.Token.AllowTokenAutoRefresh != nil {
				cfg.Token.AllowTokenAutoRefresh = *req.Token.AllowTokenAutoRefresh
			}
			if req.Token.RequireOfficialRefreshFlow != nil {
				cfg.Token.RequireOfficialRefreshFlow = *req.Token.RequireOfficialRefreshFlow
			}
		...
		if req.Token != nil {
			...
			if req.Token.MaxChatQuota != nil {
				dbUpdates["token.max_chat_quota"] = fmt.Sprintf("%d", *req.Token.MaxChatQuota)
			}
			if req.Token.MaxImageQuota != nil {
				dbUpdates["token.max_image_quota"] = fmt.Sprintf("%d", *req.Token.MaxImageQuota)
			}
			if req.Token.MaxVideoQuota != nil {
				dbUpdates["token.max_video_quota"] = fmt.Sprintf("%d", *req.Token.MaxVideoQuota)
			}
			if req.Token.AllowUnlimitedQuota != nil {
				dbUpdates["token.allow_unlimited_quota"] = fmt.Sprintf("%t", *req.Token.AllowUnlimitedQuota)
			}
			if req.Token.TokenHealthCheckIntervalMinutes != nil {
				dbUpdates["token.token_health_check_interval_minutes"] = fmt.Sprintf("%d", *req.Token.TokenHealthCheckIntervalMinutes)
			}
			if req.Token.TokenExpiryWarningHours != nil {
				dbUpdates["token.token_expiry_warning_hours"] = fmt.Sprintf("%d", *req.Token.TokenExpiryWarningHours)
			}
			if req.Token.TokenCriticalExpiryHours != nil {
				dbUpdates["token.token_critical_expiry_hours"] = fmt.Sprintf("%d", *req.Token.TokenCriticalExpiryHours)
			}
			if req.Token.AllowTokenAutoRefresh != nil {
				dbUpdates["token.allow_token_auto_refresh"] = fmt.Sprintf("%t", *req.Token.AllowTokenAutoRefresh)
			}
			if req.Token.RequireOfficialRefreshFlow != nil {
				dbUpdates["token.require_official_refresh_flow"] = fmt.Sprintf("%t", *req.Token.RequireOfficialRefreshFlow)
			}
			if req.Token.QuotaRecoveryMode != nil {
