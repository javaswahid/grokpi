...
func TestRouteBodyLimit(t *testing.T) {
	...
	tests := []struct {
		...
		{http.MethodPost, "/v1/video/generations", cfg.App.ChatBodyLimit},
...
func TestRouteBodyLimit_CustomConfig(t *testing.T) {
	...
	if got := routeBodyLimit(cfg, http.MethodPost, "/v1/video/generations"); got != 20<<20 {
		t.Errorf("custom video generation body limit = %d, want %d", got, 20<<20)
	}
}
