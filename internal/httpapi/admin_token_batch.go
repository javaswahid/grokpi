...
type BatchTokenRequest struct {
	...
	QuotaMode   string         `json:"quota_mode,omitempty"`   // limited, daily_reset, unlimited, manual
...
// resolveImportQuota resolves the legacy chat quota for older call sites/tests.
...
func resolveImportQuota(quota *int, cfg *config.TokenConfig) int {
	return resolveImportQuotaForCategory(token.CategoryChat, quota, cfg, token.QuotaModeLimited)
}

// resolveImportQuotaForCategory resolves a category-specific quota for an imported token.
// If quota is nil or *quota <= 0, auto-resolve based on config defaults.
func resolveImportQuotaForCategory(cat token.QuotaCategory, quota *int, cfg *config.TokenConfig, quotaMode string) int {
	if quota != nil && *quota > 0 {
		return token.ClampQuota(cat, *quota, cfg, quotaMode)
	}
	if cfg != nil {
		switch cat {
		case token.CategoryImage:
			if cfg.DefaultImageQuota > 0 {
				return token.ClampQuota(cat, cfg.DefaultImageQuota, cfg, quotaMode)
			}
		case token.CategoryVideo:
			if cfg.DefaultVideoQuota > 0 {
				return token.ClampQuota(cat, cfg.DefaultVideoQuota, cfg, quotaMode)
			}
		default:
			if cfg.DefaultChatQuota > 0 {
				return token.ClampQuota(cat, cfg.DefaultChatQuota, cfg, quotaMode)
			}
		}
	}
	switch cat {
...
func handleBatchImport(ctx context.Context, ts TokenStoreInterface, syncer TokenPoolSyncer, req BatchTokenRequest, cfg *config.TokenConfig) BatchTokenResponse {
	...
	quotaMode := token.NormalizeQuotaMode(req.QuotaMode)
	...
	for i, tokenStr := range req.Tokens {
		...
		chatQuotaReq := req.ChatQuota
		if chatQuotaReq == nil {
			chatQuotaReq = req.Quota // backward compatibility with old quota field
		}
		chatQ := resolveImportQuotaForCategory(token.CategoryChat, chatQuotaReq, cfg, quotaMode)
		imageQ := resolveImportQuotaForCategory(token.CategoryImage, req.ImageQuota, cfg, quotaMode)
		videoQ := resolveImportQuotaForCategory(token.CategoryVideo, req.VideoQuota, cfg, quotaMode)
		token := &store.Token{
			Token:             tokenStr,
			Pool:              req.Pool,
			QuotaMode:         quotaMode,
			...
			RefreshStatus:     token.RefreshStatusIdle,
			TokenVersion:      1,
		}
...
func handleRefreshToken(tr TokenRefresher) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if errors.Is(err, token.ErrPolicyQuarantine) {
				WriteError(w, 409, "policy_quarantine", "prompt_repair_required",
					"Token is in policy quarantine. Repair and validate the prompt before reuse.")
				return
			}
...
// handleRevalidateToken returns a handler that validates a token without resetting usage.
func handleRevalidateToken(tv TokenRevalidator) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		idStr := chi.URLParam(r, "id")
		id, err := strconv.ParseUint(idStr, 10, 32)
		if err != nil {
		...
		refreshed, err := tv.RevalidateToken(r.Context(), uint(id))
		if err != nil {
			if errors.Is(err, token.ErrTokenNotFound) {
				WriteError(w, 404, "not_found", "token_not_found",
					"Token not found")
			...
			slog.Warn("failed to revalidate token", "id", id, "error", err)
			WriteError(w, 502, "upstream_error", "revalidate_failed",
				"Failed to revalidate token with upstream")
			return
		}
...
// handleOfficialRefreshToken returns a handler for official token refresh.
// In v1 this is a safe stub unless a supported official refresh provider is wired.
func handleOfficialRefreshToken(tr TokenOfficialRefresher) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		idStr := chi.URLParam(r, "id")
		id, err := strconv.ParseUint(idStr, 10, 32)
		if err != nil {
		...
		refreshed, err := tr.RefreshTokenOfficial(r.Context(), uint(id))
		if err != nil {
			if errors.Is(err, token.ErrTokenNotFound) {
				WriteError(w, 404, "not_found", "token_not_found",
					"Token not found")
			...
			WriteError(w, 501, "server_error", "refresh_unavailable",
				"Auto refresh tidak tersedia. Gunakan Replace Token.")
			return
		}

		resp := tokenToResponse(refreshed)
		resp.WarningMessage = "Auto refresh tidak tersedia. Gunakan Replace Token."
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	}
}
