...
func (h *Handler) handleChatVideo(w http.ResponseWriter, r *http.Request, req *ChatRequest) {
	...
	videoCtx := flow.WithVideoTrace(httpapi.BridgeFlowContext(r.Context()), trace)
	flow.LogVideoStage(videoCtx, "request_received", "model", req.Model)
	flow.LogVideoStage(videoCtx, "auth_passed", "model", req.Model)
	videoURL, err := h.VideoFlow.GenerateSync(videoCtx, videoReq)
	if err != nil {
		var videoErr *flow.VideoError
		if errors.As(err, &videoErr) {
			status := videoErr.Status
			if status == 0 {
				status = http.StatusServiceUnavailable
			}
			h.writeStreamingOrJSONErrorWithCode(w, req.Stream, status, "server_error", videoErr.Code, videoErr.Error())
			return
		}
	...
	flow.LogVideoStage(videoCtx, "response_sent", "video_url", videoURL)
	eventCh := singleMessageEventCh(content)
	if isStreamEnabled(req.Stream) {
		h.streamResponse(w, r, eventCh, req)
		return
