...
// SelectionDiagnostics returns safe token selector diagnostics for a pool/category.
func (s *TokenService) SelectionDiagnostics(pool string, cat QuotaCategory) SelectionDiagnostics {
	return s.manager.SelectionDiagnostics(pool, cat)
}
...
func (s *TokenService) RefreshToken(ctx context.Context, id uint) (*store.Token, error) {
	...
	if Status(token.Status) == StatusPolicyQuarantine {
		return nil, ErrPolicyQuarantine
	}
...
// RevalidateToken checks upstream token validity without resetting local usage counters.
func (s *TokenService) RevalidateToken(ctx context.Context, id uint) (*store.Token, error) {
	token := s.manager.GetToken(id)
	if token == nil {
		return nil, ErrTokenNotFound
	}
	...
	if err := s.manager.ValidateToken(ctx, token, s.baseURL); err != nil {
		return nil, err
	}
	return token, nil
}
...
func (s *TokenService) ReportError(id uint, reason string) {
	if IsPolicyViolationReason(reason) {
		s.manager.MarkPolicyQuarantine(id, reason)
		return
	}
...
// RefreshTokenOfficial is a safe v1 stub unless an official refresh provider is wired later.
func (s *TokenService) RefreshTokenOfficial(ctx context.Context, id uint) (*store.Token, error) {
	token := s.manager.GetToken(id)
	if token == nil {
		return nil, ErrTokenNotFound
	}
	s.manager.MarkRefreshUnavailable(id, "Auto refresh tidak tersedia. Gunakan Replace Token.")
	return token, nil
}
...
func (s *TokenService) FlushDirty(ctx context.Context) error {
	...
	for i, d := range dirty {
		...
		snapshots[i] = store.TokenSnapshotData{
			...
			LastRefreshAttemptAt: d.LastRefreshAttemptAt,
