...
func TestQuota_Consume(t *testing.T) {
	...
	t.Run("marks token quota_exhausted when quota reaches zero", func(t *testing.T) {
		tok := m.GetToken(1)
		tok.ChatQuota = 1
		tok.ImageQuota = 0
		tok.VideoQuota = 0
		...
		if tok.Status != string(StatusQuotaExhausted) {
			t.Errorf("expected status=quota_exhausted, got %s", tok.Status)
		}
		if tok.CoolUntil != nil {
			t.Error("expected CoolUntil to stay nil")
		}
	})
...
func TestQuota_SyncQuota(t *testing.T) {
	...
	t.Run("sets active token to quota_exhausted when quota is zero", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			resp := RateLimitsResponse{
				RemainingQueries:  0,
				WindowSizeSeconds: 7200,
		...
		if token.Status != string(StatusQuotaExhausted) {
			t.Errorf("expected status=quota_exhausted, got %s", token.Status)
		}
		if token.CoolUntil != nil {
			t.Error("expected CoolUntil to stay nil")
		}
