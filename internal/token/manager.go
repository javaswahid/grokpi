...
type TokenManager struct {
	...
	proxyFunc func() *config.ProxyConfig
	mu        sync.RWMutex
}
...
// SetProxyProvider sets a dynamic proxy configuration provider.
func (m *TokenManager) SetProxyProvider(fn func() *config.ProxyConfig) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.proxyFunc = fn
}
...
func (m *TokenManager) AddToken(token *store.Token) {
	...
	token.QuotaMode = NormalizeQuotaMode(token.QuotaMode)
	if token.TokenVersion <= 0 {
		token.TokenVersion = 1
	}
	if token.RefreshStatus == "" {
		token.RefreshStatus = RefreshStatusIdle
	}
...
func (m *TokenManager) MarkSuccess(id uint) {
	...
	token.StatusReason = ""
	if token.RefreshStatus == "" || token.RefreshStatus == RefreshStatusFailed || token.RefreshStatus == RefreshStatusRequired {
...
func (m *TokenManager) MarkFailed(id uint, reason string) {
	...
	if IsPolicyViolationReason(reason) {
		m.markPolicyQuarantineLocked(token, reason)
		return
	}
...
// MarkPolicyQuarantine keeps a token out of automatic recovery after a provider
// policy/safety rejection. The prompt must be repaired before an operator clears it.
func (m *TokenManager) MarkPolicyQuarantine(id uint, reason string) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *TokenManager) markPolicyQuarantineLocked(token *store.Token, reason string) {
	token.Status = string(StatusPolicyQuarantine)
	token.StatusReason = reason
	token.CoolUntil = nil
...
func (m *TokenManager) MarkExpired(id uint, reason string) {
	...
	token.ExpiresAt = &now
	token.RefreshStatus = RefreshStatusRequired
	token.RefreshError = reason
	m.dirty[id] = struct{}{}
}

// MarkInvalid transitions a token to invalid state.
func (m *TokenManager) MarkInvalid(id uint, reason string) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// MarkQuotaExhausted marks a token as locally quota exhausted.
func (m *TokenManager) MarkQuotaExhausted(id uint, reason string) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// MarkRefreshUnavailable records that no official refresh mechanism is configured.
func (m *TokenManager) MarkRefreshUnavailable(id uint, reason string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	token.RefreshStatus = RefreshStatusUnavailable
	token.RefreshError = reason
	token.RefreshCount++
	if Status(token.Status) == StatusRefreshRequired || Status(token.Status) == StatusRefreshFailed {
	...
	for id := range m.dirty {
		if token, ok := m.tokens[id]; ok {
			...
			if token.ExpiresAt != nil {
				t := *token.ExpiresAt
				snapshot.ExpiresAt = &t
			}
			if token.LastValidatedAt != nil {
				t := *token.LastValidatedAt
				snapshot.LastValidatedAt = &t
			}
			if token.LastRefreshAttemptAt != nil {
				t := *token.LastRefreshAttemptAt
				snapshot.LastRefreshAttemptAt = &t
			}
			if token.ReplacedAt != nil {
				t := *token.ReplacedAt
				snapshot.ReplacedAt = &t
			}
...
// GetHealthCheckTokens returns tokens that should be periodically revalidated.
func (m *TokenManager) GetHealthCheckTokens() []TokenSnapshot {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	for _, token := range m.tokens {
		...
		case StatusActive, StatusCooling, StatusQuotaExhausted, StatusRefreshRequired, StatusRefreshFailed:
		default:
			continue
		}
		snapshot := TokenSnapshot{
...
func (m *TokenManager) RestoreToken(id uint, chatQuota, imageQuota, videoQuota int) {
	...
	if Status(t.Status) == StatusPolicyQuarantine {
		return
	}
	...
	t.CoolUntil = nil
	t.ExpiresAt = nil
	t.QuotaMode = NormalizeQuotaMode(t.QuotaMode)
	if t.TokenVersion <= 0 {
		t.TokenVersion = 1
	}
	if t.RefreshStatus == "" || t.RefreshStatus == RefreshStatusFailed || t.RefreshStatus == RefreshStatusRequired {
		t.RefreshStatus = RefreshStatusValidated
		t.RefreshError = ""
	}
	m.dirty[id] = struct{}{}
}
