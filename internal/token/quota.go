...
func (m *TokenManager) fetchRateLimits(ctx context.Context, authToken, baseURL string) (*RateLimitsResponse, error) {
	...
	// Resolve profile
	var profile tls_client.ClientProfile
	switch strings.ToLower(browserProfile) {
	case "firefox_102":
		profile = tls_client.Firefox_102
	case "firefox_117":
		profile = tls_client.Firefox_117
	case "chrome_103":
		profile = tls_client.Chrome_103
	case "chrome_111":
		profile = tls_client.Chrome_111
	case "chrome_112":
		profile = tls_client.Chrome_112
	case "chrome_116":
		profile = tls_client.Chrome_116
	case "chrome_117":
		profile = tls_client.Chrome_117
	case "chrome_118":
		profile = tls_client.Chrome_118
	case "chrome_119":
		profile = tls_client.Chrome_119
	case "chrome_120":
		profile = tls_client.Chrome_120
	default:
		profile = tls_client.Chrome_120
	}
