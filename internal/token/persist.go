...
func (p *Persister) updateToken(tx *gorm.DB, snapshot TokenSnapshot) error {
	return tx.Model(&store.Token{}).Where("id = ?", snapshot.ID).Updates(map[string]interface{}{
		...
		"last_validated_at":       snapshot.LastValidatedAt,
		"last_refresh_attempt_at": snapshot.LastRefreshAttemptAt,
		"refresh_status":          snapshot.RefreshStatus,
		"refresh_error":           snapshot.RefreshError,
		"refresh_count":           snapshot.RefreshCount,
		"token_version":           snapshot.TokenVersion,
		"replaced_at":             snapshot.ReplacedAt,
	}).Error
}
