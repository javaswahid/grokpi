...
func (p *TokenPool) selectWithExclude(algorithm string, cat QuotaCategory, exclude map[uint]struct{}) *store.Token {
	...
	for _, t := range p.tokens {
		...
		if reason := TokenRejectionReason(t, cat, nil, now); reason != "" {
			continue
		}
...
func (p *TokenPool) Count() (active, cooling, disabled, expired int) {
	...
	for _, t := range p.tokens {
		...
		case StatusExpired:
			expired++
		case StatusQuotaExhausted, StatusInvalid, StatusRefreshRequired, StatusRefreshFailed, StatusPolicyQuarantine, StatusRateLimited, StatusNeedsManualCheck:
			disabled++
		}
	}
	return
...
func isSelectableStatus(status Status) bool {
	return status == StatusActive || status == StatusQuotaExhausted
}
