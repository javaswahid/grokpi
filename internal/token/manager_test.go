...
func TestManager_MarkFailed_PolicyViolationQuarantines(t *testing.T) {
	cfg := &config.TokenConfig{FailThreshold: 3}
	mgr := NewTokenManager(cfg)

	mgr.AddToken(&store.Token{ID: 1, Token: "t1", Pool: PoolBasic, Status: string(StatusActive), ChatQuota: 80})

	mgr.MarkFailed(1, "policy violation: unsafe prompt")

	token := mgr.GetToken(1)
	require.NotNil(t, token)
	assert.Equal(t, string(StatusPolicyQuarantine), token.Status)
	assert.Equal(t, "policy violation: unsafe prompt", token.StatusReason)
	assert.Nil(t, token.CoolUntil)
}
...
func TestRestoreToken_DoesNotRestorePolicyQuarantine(t *testing.T) {
	cfg := &config.TokenConfig{FailThreshold: 3}
	mgr := NewTokenManager(cfg)

	mgr.AddToken(&store.Token{
		ID: 1, Token: "policy1", Pool: PoolBasic,
		Status: string(StatusPolicyQuarantine), StatusReason: "content blocked by safety filter",
		ChatQuota: 0, ImageQuota: 0, VideoQuota: 0,
	})
	...
	assert.Equal(t, string(StatusPolicyQuarantine), tok.Status)
	assert.Equal(t, 0, tok.ChatQuota)
	assert.Equal(t, 0, tok.ImageQuota)
	assert.Equal(t, 0, tok.VideoQuota)
}
